  ...
  return (
    <div style={{ background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: "12px", display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", padding: "20px 24px 16px", borderBottom: "1px solid #f1f5f9", flexShrink: 0, gap: 16 }}>
        <div>
          <p style={{ fontFamily: FF, fontSize: 14, fontWeight: 700, color: "#0f172a", margin: "0 0 4px" }}>Machines Requiring Inspection Attention</p>
          <p style={{ fontFamily: FF, fontSize: 11, color: "#94a3b8", margin: 0 }}>MHE units with warning or critical findings from recent inspections.</p>
        </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px", display: "flex", flexDirection: "column", gap: 12 }}>
        {rows.length === 0 ? (
          ...
          return (
            <div key={row.mheId} style={{ border: "1px solid #e2e8f0", borderRadius: 12, padding: "14px 18px", display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ fontFamily: FF, fontSize: 11, color: "#64748b", margin: 0 }}>Last Inspection: {date}</p>
                </div>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-around", paddingTop: 10, borderTop: "1px solid #f1f5f9" }}>
                {COLS.map(({ label, key, color }) => (
                  <div key={label} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                    <span style={{ fontFamily: FF, fontSize: 11, color: "#94a3b8" }}>{label}</span>
                    <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                      <div style={{ width: 8, height: 8, borderRadius: "50%", background: color, flexShrink: 0 }} />
                      <span style={{ fontFamily: FF, fontWeight: 600, fontSize: 14, color: "#0f172a" }}>{(row as any)[key]}</span>
                    </div>
                  </div>
                ))}
              </div>
