...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: "6px", padding: "10px 14px", boxShadow: "0 4px 12px rgba(0,0,0,0.08)", minWidth: "200px" }}>
        ...
        <div>
          ...
          {redVisible.map(([mhe, val]) => (
            <div key={mhe} style={{ display: "flex", justifyContent: "space-between", gap: "8px", marginBottom: "3px" }}>
              ...
              <span style={{ fontFamily: "Inter, sans-serif", fontSize: "10px", fontWeight: 600, color: "var(--w-text-1)" }}>{val}</span>
            </div>
          ))}
        </div>
        {/* Amber column */}
      ...
      {remaining > 0 && (
        <p style={{ fontFamily: "Inter, sans-serif", fontSize: "9px", color: "var(--w-text-3)", margin: "6px 0 0" }}>+{remaining} more MHEs</p>
      )}
    </div>
  );
}
    ...
    <div style={{ background: "var(--w-bg)", border: "1px solid var(--w-border)", borderRadius: "12px", display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "13px 16px 14px 16px", borderBottom: "1px solid var(--w-divider)", flexShrink: 0, height: "81px", boxSizing: "border-box" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
          <span style={{ fontFamily: "Inter, sans-serif", fontWeight: 700, fontSize: "12px", lineHeight: "18px", color: "var(--w-text-1)", whiteSpace: "nowrap" }}>Top MHEs with Findings</span>
          <span style={{ fontFamily: "Inter, sans-serif", fontWeight: 400, fontSize: "10px", lineHeight: "15px", color: "var(--w-text-2)", whiteSpace: "nowrap" }}>Red vs Amber severity trend over time</span>
        </div>
          ...
          <div style={{ width: "100%", height: "100%" }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={data} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
                ...
                />
                <Tooltip content={<CustomTooltip />} cursor={{ stroke: "var(--w-border)", strokeWidth: 1 }} />
                <Line
                  type="monotone"
                  dataKey="red"
                  stroke="#ef4444"
      ...
      <div style={{ display: "flex", gap: "16px", padding: "8px 16px 12px", justifyContent: "center" }}>
        {([["Red Severity", "#ef4444"], ["Amber Severity", "#f59e0b"]] as [string, string][]).map(([label, color]) => (
          <span key={label} style={{ display: "flex", alignItems: "center", gap: "6px", fontFamily: "Inter, sans-serif", fontSize: "10px", color: "var(--w-text-2)" }}>
